...
func (s *LambingService) Create(ctx context.Context, userID, sheepID string, l domain.Lambing) error {
	return s.repo.AddLambing(ctx, userID, sheepID, l)
}

func (s *LambingService) List(ctx context.Context, userID string, from, to *time.Time) ([]domain.Lambing, error) {
	return s.repo.FilterLambings(ctx, userID, from, to)
}

func (s *LambingService) Update(ctx context.Context, userID, sheepID string, index int, l domain.Lambing) error {
	return s.repo.UpdateLambing(ctx, userID, sheepID, index, l)
}

func (s *LambingService) Delete(ctx context.Context, userID, sheepID string, index int) error {
